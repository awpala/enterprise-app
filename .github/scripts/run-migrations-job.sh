#!/usr/bin/env bash
# Starts the EF Core migrations Container Apps Job and polls to completion.
# Expects terraform state available under infra/ and az CLI logged in.
set -euo pipefail

RG=$(terraform -chdir=infra output -raw resource_group_name)
JOB=$(terraform -chdir=infra output -raw migrations_job_name)
