#!/usr/bin/env bash
# Builds and pushes container images via az acr build.
...
#
# Requires terraform state available under infra/ and az CLI authenticated.
set -euo pipefail
...
ACR_LOGIN_SERVER=$(terraform -chdir="${GITHUB_WORKSPACE}/infra" output -raw acr_login_server)
