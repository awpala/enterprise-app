...
#   eval "$(bash docs/runbooks/scripts/azure-source-sso-env.sh production)"
#
#   Then:
...
#     gitignored `docs/runbooks/scripts/azure-push-sso-secrets.sh` (copied from
#     `sample.azure-push-sso-secrets.sh`). The operator populates secret values
...
#   - The sourced `azure-push-sso-secrets.sh` normally runs `gh secret set` /
...
if [[ "$ENV" != "dev" && "$ENV" != "production" ]]; then
  echo "Usage: azure-source-sso-env.sh <dev|production>" >&2
  echo "  eval \"\$(bash docs/runbooks/scripts/azure-source-sso-env.sh dev)\"" >&2
  exit 1
fi
...
if [[ ! -f "$SECRETS" ]]; then
  ...
  echo "Copy docs/runbooks/scripts/sample.azure-push-sso-secrets.sh to azure-push-sso-secrets.sh and populate it first." >&2
  exit 1
fi
...
(
  ...
  # docs/runbooks/azure-sso-manual-bootstrap.md). The root module no longer reads
