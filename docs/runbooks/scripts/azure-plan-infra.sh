...
#   `infra/azure/` module. Replaces the multi-step "eval source-sso-env + az account
#   show + cd infra + terraform init + terraform plan" sequence documented in
#   Section 8 (Part F) of docs/runbooks/azure-sso-manual-bootstrap.md. Targets a
#   single environment per invocation — `dev` or `production`.
#
# Usage:
#   bash docs/runbooks/scripts/azure-plan-infra.sh dev
#   bash docs/runbooks/scripts/azure-plan-infra.sh production
#
...
#   bash docs/runbooks/scripts/azure-plan-infra.sh production -o /tmp/prod.tfplan
#
#   Runnable from any working directory — the script resolves its own
#   location and `cd`s into /workspace/infra/azure internally.
#
# Prerequisites:
...
#     This script reuses azure-source-sso-env.sh — the single source of truth for
...
#   force a refresh locally, run Terraform from `infra/azure/` by hand.
#
...
#   1   argument validation, missing az login, missing azure-push-sso-secrets.sh,
...
usage() {
  cat >&2 <<'USAGE'
Usage: azure-plan-infra.sh <dev|production> [--out <file> | -o <file>]

Examples:
  bash docs/runbooks/scripts/azure-plan-infra.sh dev
  bash docs/runbooks/scripts/azure-plan-infra.sh production --out /tmp/prod.tfplan
USAGE
}
...
SOURCE_SSO_ENV="$SCRIPT_DIR/azure-source-sso-env.sh"
SECRETS="$SCRIPT_DIR/azure-push-sso-secrets.sh"
INFRA_DIR="$(cd "$SCRIPT_DIR/../../../infra/azure" && pwd)"
...
# Reuse azure-source-sso-env.sh verbatim — it is the single place that knows how to
# extract values from azure-push-sso-secrets.sh.
