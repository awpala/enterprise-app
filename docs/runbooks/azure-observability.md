# Azure Observability Adapter - Verify, Query, and Operate

## 1. Overview

This runbook covers post-deploy verification and day-2 operation of the Azure observability adapter adopted in [ADR 0001](../adrs/0001-azure-native-observability.md) and generalized by [ADR 0003](../adrs/0003-multicloud-application-generalization.md). Scope: Application Insights for the API and data engine, Next.js server OpenTelemetry registration, platform diagnostics for PostgreSQL and ACR, and two portal-native workbooks. Use the [AWS workbook](../workbooks/aws-deployment-workbook.md) for CloudWatch/X-Ray operations.
...
4. Expect the instrumented backend path as connected nodes:
   - `ea-api`
   - `ea-data-engine`
   ...
   The `ea-ui` server appears only when an OTLP exporter is configured for the Next.js runtime. Browser real-user monitoring is not implemented. If an expected backend node is missing, go to [Section 8](#8-troubleshooting-no-telemetry).
...
**A. Recent traces from the instrumented backend services** - confirms the `cloud_RoleName` discriminator works:

```kql
traces
| where cloud_RoleName in ("ea-api", "ea-data-engine")
| take 50
| project timestamp, cloud_RoleName, message
```

Expect both role names after an end-to-end run. If one is absent, that service is not exporting.
...
select action, entity_type, entity_id, actor_name, actor_subject_id, actor_identity_provider, occurred_at_utc
from audit_events
...
- `actor_subject_id` - normalized subject (the Entra adapter preserves GUID `oid` values)
- `actor_tenant_id` - normalized tenant/issuer identifier (the Entra adapter preserves GUID `tid` values)
- `actor_identity_provider` - normalized upstream provider (values include `"google.com"`, `"EmailOTP"`, `"guest"`, and `"dev"`)
...
FROM audit_events
WHERE actor_subject_id = '<normalized-subject-id>'
ORDER BY occurred_at_utc DESC;
```
...
Common offenders: `traces` (lower a noisy logger from `Information` to `Warning`), `dependencies` (turn off SQL command-text capture), or platform diagnostic categories you do not actually use - prune those at the diagnostic-setting level in [`infra/azure/modules/diagnostics/main.tf`](../../infra/azure/modules/diagnostics/main.tf).
...
### Send a failing API request and confirm it lands under `ea-api`

1. Send an authenticated request with an intentionally invalid resource identifier to a dev API route.
2. In the App Insights blade, left nav -> **Investigate** group -> **Failures**. Filter **Role** to `ea-api`, locate the failed request, and confirm its operation ID matches the correlated structured log entry.
...
If empty, the latest `terraform apply` did not wire the App Insights output through to the Container App env. Re-check [`infra/azure/outputs.tf`](../../infra/azure/outputs.tf) and the consuming Container App module wiring.

### Is the Next.js server healthy?

The UI is now a standalone server container with request-time configuration, not a build-time telemetry-injected SPA. Verify its health and runtime configuration without expecting a connection string in browser JavaScript:

```bash
APP_URL=$(terraform -chdir=infra/azure output -raw application_url)
curl --fail "$APP_URL/api/health"
curl --fail "$APP_URL/api/runtime-config" | jq '{deploymentTarget, apiUrl, provider: .auth.provider}'
```

Provider-neutral browser real-user monitoring is a documented follow-up gap. Do not expose the Application Insights connection string through `/api/runtime-config`.

### Is the exporter selection explicit?

Both services require `OBSERVABILITY_EXPORTER=azuremonitor` (mapped to `Observability__Exporter` for the API) in addition to the Application Insights connection string. The data engine selects `configure_azure_monitor(...)`; the API selects `UseAzureMonitor(...)`. Tail container logs and look for exporter initialization errors:
...
The API fails fast when Azure Monitor is selected without a connection string. The data engine likewise validates its selected exporter in settings. An unexpected clean startup with no telemetry usually means the exporter remains `none` or the latest Terraform revision was not activated.
...
For the Next.js server, confirm `OTEL_SERVICE_NAME=ea-ui` and the registration in [`ui/instrumentation.ts`](../../ui/instrumentation.ts).

### Is the LAW receiving platform diagnostics?

If app telemetry works but PostgreSQL or ACR diagnostic categories are missing in Log Analytics, confirm the diagnostic settings actually applied:

```bash
az monitor diagnostic-settings list \
  --resource <full-resource-id-of-postgres-or-acr> \
  --query "[].{name:name, logs:logs[].category, metrics:metrics[].category}"
```

Expect to see a setting whose categories match what [`infra/azure/modules/diagnostics/main.tf`](../../infra/azure/modules/diagnostics/main.tf) declares for that resource type. If the list is empty, re-run `terraform apply` and check for errors on the diagnostics module - a common cause is a stale resource ID after a resource was recreated.
