...
echo "Pushing SSO values to GitHub Environment: $ENV"

gh secret   set EXTERNAL_TENANT_ID            --env "$ENV" --body "$EXTERNAL_TENANT_ID"
gh variable set TENANT_SUBDOMAIN              --env "$ENV" --body "$TENANT_SUBDOMAIN"
gh secret   set EXTERNAL_TENANT_CLIENT_ID     --env "$ENV" --body "$EXTERNAL_TENANT_CLIENT_ID"
gh secret   set EXTERNAL_TENANT_CLIENT_SECRET --env "$ENV" --body "$EXTERNAL_TENANT_CLIENT_SECRET"

echo "Done — verify with: gh secret list --env $ENV && gh variable list --env $ENV"
