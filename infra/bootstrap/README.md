...
- An **Entra ID application + service principal** (`ea-github-oidc`) with federated credentials for `dev` and `production` GitHub Environments — no client secrets, no passwords.
...
This populates repo secrets (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`), repo variables (`TFSTATE_RESOURCE_GROUP`, `TFSTATE_STORAGE_ACCOUNT`, `TFSTATE_CONTAINER`), and creates the `dev` / `production` GitHub Environments.
